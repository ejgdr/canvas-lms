...
Circuit breaker (P1 hardening) wraps `@client.summarize` in `SummarizationService`. Three-key Redis state machine per account global_id: `open_key` (no TTL, deleted on close), `cooldown_key` (TTL = cooldown, expires to signal half-open), `probe_key` (SET nx — admits exactly one concurrent probe). Opens after N consecutive `TransportError` raises (Setting `discussion_thread_summarizer_circuit_failure_threshold`, default 5); half-open when `cooldown_key` expires; single probe via `probe_key` nx; re-opens on probe failure. Open circuit serves cached rows (current/stale) and blocks only generation: `lookup_for_render` checks circuit AFTER record fetch; `fetch_or_create_summary` checks circuit BEFORE rate-limiter to avoid burning budget. Emits `discussion_thread_summarizer.circuit_open` / `circuit_closed` (frozen names for Cycle 37 #50). Outage spec (#49) drives N real failures via `enqueue_for + run_jobs`; asserts single-failure renders `:generating`, N-failure opens circuit and subsequent GET returns `:unavailable` with zero client calls, and existing `:current` row is served despite open circuit.
